...
PERSONAS = {
    "brief_report": {
        ...
        "desc": "첫 문장은 종목명과 숫자 하나만으로 짧게 끝냅니다. "
                # 예시에 구체 수치를 넣으면 그대로 복제돼 근거없는수치로 리젝된다
                "종목명을 먼저 쓰고 입력에 있는 수치 하나를 붙이는 형태입니다.\n"
                "두 번째 문장에서 그 숫자의 배경을 밝힙니다.\n"
                "종결어미는 '~습니다' 와 '~네요' 를 섞습니다. 세 문장이 모두 "
                "같은 어미로 끝나지 않게 합니다.\n"
                "형용사와 부사를 쓰지 않습니다.\n"
    ...
    "term_guide": {
        ...
        "num_cap": 3, "no_question": True,
        "desc": "숫자나 용어 **하나**를 골라, 이 건에서 그것이 무엇을 가리키는지 한 문장으로 풀어 줍니다.\n"
                "일반적인 사전식 정의는 쓰지 않습니다. 이 사안에 한정해 설명합니다.\n"
    ...
    "data_focus": {
        ...
        "desc": "입력에 이미 계산되어 있는 비교값(평균 대비 배수, 누적 수익률 등)을 축으로 씁니다.\n"
                ...
                # 4문장 전부 '~습니다' 면 어미반복(4회) 필터에 걸린다. 실측 리젝 1위였다.
                "종결어미는 '~습니다' 위주로 하되 한 번은 '~인데요' 를 섞습니다.",
    },
    "careful_note": {
    ...
    "quick_memo": {
        ...
        "desc": "아주 짧게 끝냅니다. 배경 설명 없이 사실 하나와 짧은 반응만.\n"
                "크기를 평가하지 않습니다. 대단하다, 크다, 이례적이다 같은 말은 쓰지 않습니다.\n"
...
- 모든 문장을 '~습니다' 로 끝내지 마세요. 실측상 커뮤니티 글에서 '~습니다' 는
  1.7% 뿐입니다. 존댓말은 유지하되 '~네요', '~인데요', '~예요' 를 섞습니다.
...
- **수치 나열은 글이 아닙니다.** 수치가 둘 이상이면 반드시 한 번은
  둘 사이의 관계·비중·위치를 짚으세요. 이게 없으면 시세 화면과 같은 글입니다.
  쓸 수 있는 관계 (입력 안의 산술 관계이므로 평가가 아닙니다)
    · 하루 등락이 5거래일 누적에서 차지하는 비중
    · 거래량·거래대금이 평소(20일 평균) 대비 몇 배인지
    · 종가가 장중 고가·저가 사이 어디에서 끝났는지
    · 외국인·기관 순매매 순위가 몇 위인지
  예: "5거래일 누적이 6.32%인데요, 어제 하루가 그 대부분을 만든 셈입니다."
- 숫자가 없는 문장을 반드시 하나 넣습니다. 실측상 커뮤니티에서 반응이 좋은 글의
  35%는 숫자가 아예 없습니다. 숫자로만 채우면 시세 화면을 옮긴 글이 됩니다.
