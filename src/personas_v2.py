...
PERSONAS = {
    "brief_report": {
        ...
        # 원래 "숫자를 단독으로 던지고 시작" 이었으나 전역 규칙(수치 선두 금지)과
        # 정면 충돌해 이 페르소나가 통째로 리젝됐다. 결과적으로 open_talk 만
        # 살아남아 배포 23건 중 17건이 질문형이 됐다 (실측).
        "desc": "첫 문장은 종목명을 주어로 삼아 입력 사실 하나를 완전한 문장으로 씁니다. "
                "종목명과 숫자만 나열한 문장 파편은 쓰지 않습니다.\n"
                "두 번째 문장에서 그 숫자의 배경을 밝힙니다.\n"
                "형용사와 부사를 쓰지 않습니다.\n"
    ...
    "fact_note": {
        ...
        "desc": "확인된 사실을 순서대로 적고, 마지막 한 문장은 허용된 주장 목록 안에서만 덧붙입니다.\n"
                "질문으로 끝내지 않습니다. 마지막 사실에서 담담하게 끊습니다.",
    },
    "term_guide": {
        "name": "용어해설", "sentences": "5~6문장", "min": 110, "max": 280,
        "num_cap": 3, "no_question": True,
        "desc": "입력의 [용어 설명] 문장 하나를 골라 그대로 전달합니다.\n"
                "[용어 설명]이 없으면 이 페르소나는 선택되지 않습니다. 일반 지식으로 정의를 만들지 않습니다.\n"
                "여러 개를 설명하면 사전이 됩니다. 딱 하나만 고르세요.\n"
                "첫 문장은 종목명으로 시작해 무슨 일이 있었는지, 두 번째 문장에서 풀이합니다.\n"
                "",
    },
    "data_focus": {
    ...
    "careful_note": {
        ...
        "num_cap": 4, "no_question": True,
        "desc": "한 문장은 입력에 계산된 폭이나 흐름의 값을 그대로 담습니다.\n"
    ...
    "check_list": {
        ...
        "desc": "입력에서 확인된 조건·금액·시점을 두세 가지 순서대로 짚습니다.\n"
                "입력에 없는 향후 확인 과제나 미래 수치를 새로 만들지 않습니다.\n"
    ...
    "quick_memo": {
        ...
        "name": "짧은메모", "sentences": "2~3문장", "min": 55, "max": 120,
        "num_cap": 3, "no_question": True,
        # '5어절 이하' 강제가 문장을 부쉈다 (실측: "저가 대비 고가." 비문 발생).
        # 전체 길이 35~120자 제한이 이미 있어 별도 어절 강제가 필요 없다.
        "desc": "아주 짧게 끝냅니다. 배경 설명 없이 확인된 사실만 씁니다.\n"
    ...
    "open_talk": {
        ...
        "desc": "사실을 짧게 적고 마지막 줄을 **구체적인** 질문으로 끝냅니다.\n"
                ...
                # 예시를 주면 그대로 복제된다. 실측: '아시는 분 계신가요?' 로 끝난 글 7건
                "입력에 있는 항목을 지목해 묻습니다. "
                "질문 문장은 매번 다르게 만듭니다. 같은 표현을 반복하지 않습니다.\n"
                "",
    },
}
...
{persona_desc}
- 분량: {sentences}, 공백 포함 {min}자 이상 {max}자 이하.
  하한 미달은 리젝됩니다. 짧게 끝내되 {min}자는 반드시 채우세요.
...
- 첫 문장은 종목명(또는 정책·테마 이름)으로 시작합니다. 수치로 시작하지 않습니다.
  예: (X) "9.43% 올랐습니다." → (O) "파두가 9.43% 올랐습니다."
- 자료에 기준일이 있으면 그 날짜를 정확히 쓰거나 날짜 표현을 생략합니다.
  "오늘·어제·전일·전날·지난 거래일" 같은 상대 날짜는 쓰지 않습니다.
- 반드시 존댓말 구어체로 씁니다. 신문 기사체("~했다", "~이다")는 쓰지 않습니다.
- 독자를 "당신"으로 부르지 않습니다.
- 문장 길이를 불규칙하게. 같은 종결어미를 네 번 이상 반복하지 않습니다.
- 종결어미를 특정 형태로 고정하지 않습니다. 어떤 글이든 '~네요' 나 '~인데요' 를
  한 번씩 끼워 넣는 식으로 쓰지 마세요. 모든 글이 같은 버릇을 가지면 사람이 쓴
  글로 읽히지 않습니다. 사실에 맞는 어미를 그때그때 고릅니다.
- 첫 문장을 질문으로 시작하지 않습니다.
- 가장 눈에 띄는 사실 한 문장에는 느낌표를 붙여도 됩니다. 글 전체에서 한 번까지입니다.
- 'ㅎㅎ' 나 물결(~)은 글 전체에서 한 번까지 쓸 수 있습니다. 주가가 내린 글에는
  쓰지 않습니다. 'ㅋㅋ', 'ㅡㅡ', 'ㅠㅠ' 와 이모지는 쓰지 않습니다.
...
- 서로 다른 숫자는 {num_cap}개까지만 씁니다.
- 미확인 정보는 원칙적으로 언급하지 않습니다. "상세 수치는 확인되지 않았습니다" 류
  빈 문장으로 하한을 채우지 마세요. 하한은 사실을 더 적어서 채웁니다.
- **[결합 사실] 항목 중 최소 하나는 그 수치를 본문에 그대로 쓰세요.**
  종가·등락률 같은 raw 수치는 독자가 이미 시세 화면에서 보고 있습니다.
