...
TIER4_NOISE = [
    ...
    "일괄신고", "증권발행실적",
    # '기재정정'·'첨부정정'은 뺐다. 정정 공시에서 수치를 뽑을 방법이 없던 시절
    # 만든 규칙인데, 조회창 180일 확대와 원문 파서로 지금은 수치가 붙는다.
    # 실측: 게이트 차단 22건 중 10건이 이 규칙이었다. 규칙이 개선을 앞질렀다.
    # 수치가 안 붙는 정정 건은 has_substance 가 tier5 로 걸러낸다 — 이중으로 막을 이유가 없다.
]
