...
# 콜론까지 본다. facts.annotate_terms 가 붙이는 '용어 설명: 투자의견은 …' 줄은
# 코드가 넣은 사전 문구지 리포트가 준 값이 아니다. 이걸 글감으로 치면 목표가·
# 투자의견이 전무한 리포트가 게이트를 통과해 제목 재진술 글이 된다.
# 요지도 글감으로 인정한다. 목표가·투자의견이 없는 리포트도 내용이 있으면
# 쓸 값어치가 있다는 운영 판단이다(리서치 발송량은 유지·확대가 목표).
# #125 에서 IR 소개자료가 점령한 것은 요지를 허용해서가 아니라 요지가 회사
# 소개 보일러플레이트였기 때문이다. 그 선별은 research._gist 가 한다 —
# 실질 문장이 없으면 요지를 아예 만들지 않고, 그러면 여기서 막힌다.
_RESEARCH_SUBSTANCE = re.compile(r"제시 적정가격:|투자의견:|리포트 요지:")
