...
def check(body: str, facts: str, fmt: str = None, angle: str = None,
    ...
    # 'ㅎㅎ' 도 끝맺음으로 본다. 허용해 놓고 종결부호 규칙을 안 맞춰서
    # 실측 #139 에서 'ㅎㅎ' 로 끝난 글이 전부 미완성으로 잘렸다(사용률 0%).
    # 'ㅎㅎ' 자체가 끝맺음이다. 커뮤니티는 '…이었어요 ㅎㅎ' 처럼 마침표 없이 쓴다.
    _tail = body.strip()
    if _tail and not re.search(r"ㅎ{2,}$", _tail) and _tail[-1] not in ".!?\"')]”’…~":
