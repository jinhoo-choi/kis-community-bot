...
# 요지로 넘길 최대 길이. 원문을 통째로 싣지 않기 위한 상한이자, 본문 길이 상한과의
# 계약이다. 180자였을 때 요지 하나가 brief_report(150자)·quick_memo(120자) 본문
# 상한보다 길어, 모델이 다 담으려다 너무김·수치과다로 걸렸다.
# 실측 #139: 리서치 21건 시도 중 5건만 통과(24%), 리젝은 수치과다 12 / 너무김 10.
# 정책에서 같은 원인을 GIST_MAX 110 으로 고쳤다(#32). 여기도 본문 상한 아래로 둔다.
GIST_MAX = 130
