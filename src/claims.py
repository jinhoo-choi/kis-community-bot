...
CLAIM_SPECS = [
    ...
    # 결합 사실은 '· 라벨: 값' 한 줄로만 나온다. 줄 중간까지 잡으면 다른 유형의
    # 산문에 같은 표현이 있을 때 엉뚱한 주장이 생긴다.
    # 실측: 리서치 요지의 '고점 대비 약 50% 하락한 수준이다' 가 flow 의
    # drawdown 주장으로 잡혔다(감사 I2).
    ("move_x",    "등락 크기",     r"(?m)^·?\s*등락 크기[:\s]*([^\n]+)", "{}"),
    ("drawdown",  "고점 대비",     r"(?m)^·?\s*고점 대비[:\s]*([^\n]+)", "{}"),
    ("gapfill",   "갭 되돌림",     r"(?m)^·?\s*갭 되돌림[:\s]*([^\n]+)", "{}"),
...
def _metadata_numbers(item: dict) -> set[str]:
    ...
    for line in item.get("facts", "").splitlines():
        # 제목은 출처가 준 문장 그대로다. 본문이 제목을 인용했는데 그 안의
        # 수치가 근거없는수치로 잡히면, 코드가 준 사실을 코드가 거부하는 것이다
        # (감사 I1, 실측 #127 policy: '인상분 32%는 반도체 성과급 영향').
        if re.match(r"^(?:기준일|공시일|발행일|발간일|작성일|기사일|신청일|승인일|"
                    r"보도 시각|게시 시각|공시 시각|발행 시각|시점|제목|"
                    r"리포트 제목|공시명)\s*:", line):
            out |= _nums(line)
