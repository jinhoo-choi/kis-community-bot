...
def clean(body: str) -> str:
    ...
    b = re.sub(
        ...
        r"\2% 내려 \1에 마감\3", b)
    b = re.sub(
        r"(^|(?<=[.!?])\s+)([가-힣A-Za-z0-9&·]+?)(?:이|가)\s*거래량\s*"
        r"(20일\s*평균(?:의|\s*대비)\s*\d+(?:\.\d+)?배)",
        r"\1\2의 거래량은 \3", b, flags=re.M)
    # 실발송에서 확인된 결합 사실 비문은 값이 아니라 연결 표현만 고친다.
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)(?:까지)?\s*"
        r"(?:상승했다가|올랐다가)",
        r"장중 고저 차이는 저가 대비 \1였고", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)\s*상승폭을\s*"
        r"(?:기록)?했(는데|으며|고)",
        r"장중 고저 차이는 저가 대비 \1였\2", b)
    b = re.sub(
        r"(?:같은\s*날\s*)?장중(?:에선|에는)?\s*저가\s*대비\s*"
        r"(\d+(?:\.\d+)?%)\s*상승(?:하기도)?\s*했(습니다|어요|네요)",
        lambda m: (f"장중 고저 차이는 저가 대비 {m.group(1)}"
                   + ("였습니다" if m.group(2) == "습니다" else f"였{m.group(2)}")), b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)의\s*고저\s*차이를\s*"
        r"보이며\s*변동성이\s*컸는데",
        r"장중 고저 차이는 저가 대비 \1였고", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)의\s*고저\s*차이를\s*"
        ...
        lambda m: (f"장중 고저 차이는 저가 대비 {m.group(1)}"
                   + ("였습니다" if m.group(2) == "습니다" else f"였{m.group(2)}")), b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)의\s*고저\s*차이"
        r"(였습니다|였어요|였네요)",
        r"장중 고저 차이는 저가 대비 \1\2", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)의\s*변동폭을\s*"
        r"기록했(는데요|는데|으며|고)",
        r"장중 고저 차이는 저가 대비 \1였\2", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)\s*오른\s*폭으로\s*"
        r"변동(했습니다|했어요|했네요)",
        lambda m: (f"장중 고저 차이는 저가 대비 {m.group(1)}"
                   + (" 수준이었습니다" if m.group(2) == "했습니다"
                      else " 수준이었어요" if m.group(2) == "했어요"
                      else " 수준이었네요")), b)
    b = re.sub(
    ...
    b = re.sub(
        ...
        lambda m: (f"장중 고저 차이는 저가 대비 {m.group(1)}"
                   + ("였습니다" if m.group(2) == "습니다" else f"였{m.group(2)}")), b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)까지\s*오르며\s*"
        r"변동성을\s*보였는데",
        r"장중 고저 차이는 저가 대비 \1였고", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)(?:가)?\s*"
        r"(?:올랐던\s*만큼\s*변동성이\s*컸고|상승한\s*폭을\s*기록했으며)",
        r"장중 고저 차이는 저가 대비 \1였으며", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)의\s*고저차를\s*기록했고",
        r"장중 고저 차이는 저가 대비 \1였고", b)
    b = re.sub(
        r"장중\s*저가\s*대비\s*(\d+(?:\.\d+)?%)\s*오른\s*수준까지\s*"
        r"올랐다가\s*장중\s*고가\s*대비\s*(\d+(?:\.\d+)?%)\s*"
        r"낮은\s*곳에서\s*장을\s*마감했습니다",
        r"장중 고저 차이는 저가 대비 \1였고, 종가는 장중 고가 대비 \2 낮은 수준에서 마감했습니다", b)
    b = re.sub(
    ...
    b = re.sub(
        ...
        r"에서\s*(?:장을\s*)?마감(?:했|됐)습니다",
        r"종가는 \1이었습니다", b)
    b = re.sub(
        r"마감은\s*(장중\s*고가\s*대비\s*\d+(?:\.\d+)?%\s*낮은\s*수준)"
        r"에서\s*(\d[\d,]*원)으로\s*마감했습니다",
        r"종가는 \1인 \2이었습니다", b)
    b = re.sub(
        r"장중\s*고가에서\s*(\d+(?:\.\d+)?%)\s*낮은\s*수준에서\s*"
        r"장을\s*마감했네요",
        r"종가는 장중 고가 대비 \1 낮은 수준이었네요", b)
    b = re.sub(
        r"종가는\s*(\d[\d,]*원)이었습니다\.\s*다만\s*종가는\s*"
        r"장중\s*고가\s*대비\s*(\d+(?:\.\d+)?%)\s*낮은\s*수준이었습니다",
        r"종가는 \1이었고, 장중 고가 대비 \2 낮은 수준이었습니다", b)
    b = re.sub(
        r"거래량은\s*(20일\s*평균의\s*\d+(?:\.\d+)?배)로\s*집중했는데요",
        r"거래량은 \1였는데요", b)
    ...
    b = re.sub(r"(\d+(?:\.\d+)?%)\s*(상승|하락)\s*중인데요(?=\.$)",
               r"\1 \2했네요", b)
    b = re.sub(r"((?:상승|하락)한\s*흐름)인데요(?=\.$)", r"\1입니다", b)
    # 따옴표로 통째로 감싼 출력
