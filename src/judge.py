...
2. useful    : 읽는 사람에게 새로운 정보나 생각할 거리를 주는가
   감점 — 제목을 풀어 쓴 수준 / "상세 수치는 공개되지 않았다"가 내용의 절반
          ...
          / 사실관계에 없는 업황·수혜 일반론을 덧붙임
            (예: "이런 정책이 나오면 관련 분야가 관심사가 되곤 하니까")
...
6. fit       : 커뮤니티 적합성. **이미 그 종목을 보고 있는 사람이**
   ...
   문장이 매끄러운 것과 무관하다. 아래 기준으로 매긴다.
     1점 — raw 수치(종가·등락률·거래대금)를 옮겨 적기만 했다
     2점 — raw 수치를 여럿 전했지만 서로 독립적이다
     3점 — 입력의 [결합 사실] 을 실제로 전달했다.
            개별 수치만 봐서는 안 보이는 비교·비중·위치여야 한다
            (평균 대비 배수, 누적 대비 하루 비중, 고가 대비 마감 위치, 수급 순위).
            **관계를 뜻하는 단어만 끼워넣고 값은 raw 수치뿐이면 2점이다.**
     4점 — 시세 데이터 밖의 사실을 더했다 (공시 내용, 계약 규모, 수급 주체)
     5점 — 그 사실을 이 종목의 기존 사실과 연결해 의미까지 설명했다

   유의 — 유형마다 상한이 다른 것이 정상이다.
   특징주는 새 사실이 없으므로 3점이 많고, 공시·리포트가 4~5점을 받는다.
   모든 유형을 같은 분포로 만들려 하지 말 것.
