...
RULES: list[Rule] = [
    ...
    Rule(
        id="second_person",
        writer='독자를 "당신"으로 지칭하거나 독자의 심경을 단정 '
               '("궁금하신데요", "궁금하실 텐데요"). 한국 커뮤니티에서 시비조로 읽힌다',
        judge="독자를 '당신'으로 지칭",
        # 실측 표본(#72): "얼마나 차이가 나고 있는지 궁금하신데요?"
        # '아시는 분' 은 open_talk 의 정상 어법이라 제외한다.
        regex=r"(당신|궁금하신|궁금하실|느끼셨|보셨을 텐)",
    ),
    Rule(
    ...
    Rule(
        ...
        # 실측 표본(#72): "정부와 지자체 차원의 수출 지원이 계속되고 있습니다."
        # "~고 있습니다" 로 닫는 무정보 문장이 정규식을 통과해 심사에서 fatal 이 됐다.
        regex=r"(지켜봐야|확인이 필요|확인해\s*볼 필요|관심이 (쏠리|모이)|주목(됩니다|받)|"
              r"귀추|눈여겨볼|참고하시|살펴볼 필요|체크해\s*볼|"
              # 글 끝에서만 잡는다. 본문 중간의 같은 표현은 사실 서술일 수 있다.
              r"(계속되고|계속|이어지고|늘어나고|확산되고)\s*있습니다[.\s]*$|"
              r"이어지는 모습입니다[.\s]*$|계속될 전망입니다)",
    ),
    Rule(
        id="hedge_overuse",
    ...
    Rule(
        id="claim_out_of_scope",
        writer="허용된 주장 목록에 없는 종류의 주장. "
               "특히 등락 원인, 수급 주체 추정, 업황 수혜, 투자자 기대감, "
               "향후 전망, 회사 의도, OHLC로 알 수 없는 장중 순서·기술적 신호 추측",
        judge="허용된 사실 외의 주장(원인·수혜·전망·장중 순서·기술적 신호 추측)을 함",
        regex=r"(기대감|기대가|투자심리|투심)[이가]?\s*(반영|작용|살아|개선)|"
              r"수혜(가|를|주)|영향을 받은 것|때문[으에]|덕분에|"
              r"(으로|로) 풀이|(으로|로) 분석|전망입니다|전망이다|"
              r"(하려는|하겠다는)\s*(전략|의도|계획)(으)?로|"
              r"(상승|하락)\s*추세|수급.{0,15}(우호|불리)|"
              r"추가\s*거래량|가격.{0,12}요동|"
              r"차익\s*실현|들쭉날쭉|상승\s*여파|일부를\s*반영|"
              r"(상승여력|투자\s*매력|매력도).{0,16}(충분|현실|어느|강조)|"
              r"현재가.{0,16}(매력|현실적|어느\s*쪽)|"
              r"마감을?\s*앞두고|(?:당일|장중)\s*후반부|"
              r"거래량.{0,35}(?:가격|상승|하락).{0,24}(?:뒷받침|시사)|"
              r"(?:상승|하락)\s*모멘텀.{0,24}(?:약화|강화|유효|지속)|"
              r"(?:변동성|등락폭).{0,16}(?:컸|작았|제한적).{0,16}(?:보여|보입니다|시사)",
    ),
    Rule(
        id="unsupported_cause_question",
        writer="입력에 원인이 없는데 원인·이유·수급 주체를 추측하게 하는 질문. "
               "발제글도 입력에 있는 두 관찰값의 차이만 물을 것",
        judge="입력에 없는 원인·이유·수급 요인을 독자에게 추측하게 하는 질문",
        regex=r"(?:원인|이유|요인|배경|계기)[^.!?\n]{0,28}"
              r"(?:무엇|뭐|어느|작용|주도|있었)[^.!?\n]{0,20}\?|"
              r"(?:주도한|작용한)[^.!?\n]{0,18}(?:요인|원인)|"
              r"(?:순수\s*)?수급(?:의)?\s*결과라고[^.!?\n]{0,20}\?|"
              r"영향[^.!?\n]{0,22}어느\s*쪽",
