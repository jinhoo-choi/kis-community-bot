...
RULES: list[Rule] = [
    ...
    Rule(
        id="news_cliche",
        writer='금융기사 상투어. "~을 기록했습니다"(→ "~였습니다"), '
               '"남은 과제입니다", "이번 건에서 볼 부분은". '
               "출처를 밝히지 않은 평가 표현은 쓰지 않습니다",
        judge="금융기사 상투어 사용 (~을 기록했습니다 / 남은 과제입니다 등)",
        # '기록했습니다' 는 모델 기본 어휘라 리젝하면 재생성해도 반복된다
        # (실측: 11건 리젝 중 5건). 치환 가능한 표현은 후처리로 돌리고
        # 치환이 안 되는 상투어만 리젝한다.
        regex=r"남은 과제|볼 부분은|귀추|모멘텀이 유효",
        fatal=False,
    ),
    Rule(
        id="unsourced_eval",
        writer='주체 없는 간접 평가. "~라는 평가네요", "~라는 뜻이네요", "~로 해석됩니다". '
               "평가는 반드시 누가 한 평가인지 밝히거나, 아니면 쓰지 마세요",
        judge="평가 주체를 밝히지 않은 간접 평가 표현",
        regex=r"(라는|다는)\s*(평가|분석|해석|의미|뜻)(네요|입니다|이네요)|로 해석됩니다",
    ),
    Rule(
        id="textbook",
