...
# 용어 설명은 코드가 확정 문장으로 주입한다.
# 모델 상식으로 정의를 만들면 근거 없는 정의가 되고(실측: 무근거 용어 정의로 차단),
# 반대로 주입하는 곳이 없으면 term_guide 페르소나와 decode Angle 이 영구 미선택된다
# (점검 실측: 레포 어디에서도 '용어 설명:' 을 만들지 않아 두 경로가 사장됐다).
# 값을 평가하지 않는 사전적 서술만 둔다. 유불리·전망은 넣지 않는다.
GLOSSARY = [
    ("전환사채", "전환사채는 정해진 조건으로 주식으로 바꿀 수 있는 권리가 붙은 회사채입니다."),
    ("신주인수권부사채", "신주인수권부사채는 새로 발행되는 주식을 정해진 가격에 살 수 있는 권리가 붙은 회사채입니다."),
    ("교환사채", "교환사채는 발행회사가 보유한 다른 주식으로 바꿀 수 있는 권리가 붙은 회사채입니다."),
    ("유상증자", "유상증자는 회사가 새 주식을 발행해 자금을 받는 절차입니다."),
    ("무상증자", "무상증자는 회사 잉여금을 자본금으로 옮기며 주주에게 새 주식을 나눠 주는 절차입니다."),
    ("자기주식 취득", "자기주식 취득은 회사가 자기 회사 주식을 사들이는 것입니다."),
    ("자기주식 처분", "자기주식 처분은 회사가 보유 중이던 자기 회사 주식을 파는 것입니다."),
    ("물적분할", "물적분할은 사업부를 떼어 자회사로 만들고 모회사가 그 지분을 갖는 방식입니다."),
    ("인적분할", "인적분할은 사업부를 떼어 별도 회사로 만들고 기존 주주가 지분을 나눠 갖는 방식입니다."),
    ("흡수합병", "흡수합병은 한 회사가 다른 회사를 흡수해 하나의 법인으로 남는 방식입니다."),
    ("주식배당", "주식배당은 현금 대신 주식으로 배당을 지급하는 것입니다."),
    ("무상감자", "무상감자는 주주에게 대가를 주지 않고 자본금을 줄이는 절차입니다."),
    ("조회공시", "조회공시는 거래소가 풍문·급변 사유를 물으면 회사가 답변하는 공시입니다."),
    ("단일판매", "단일판매·공급계약 공시는 회사가 맺은 계약의 금액과 기간을 알리는 공시입니다."),
    ("적정가격", "적정가격은 증권사가 리포트에서 제시한 값이며 회사가 정한 가격이 아닙니다."),
    ("투자의견", "투자의견은 증권사가 리포트에 표기한 의견 구분입니다."),
    ("공매도", "공매도는 주식을 빌려서 판 뒤 나중에 사서 갚는 거래입니다."),
]

# 시세 나열(flow)·발제(poll)에는 붙이지 않는다. 설명할 사건 자체가 없다.
