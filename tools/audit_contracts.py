...
# 페르소나 지시와 어미 필터가 서로 모순되지 않는지 본다.
# brief_report 가 '수치 선두' 로 통째로 죽었던 사고와 같은 유형이다.
#
# 2026-09-22: '전 문장 격식체' 를 리젝하던 어미단조를 뺐다. 당사 커뮤니티
# 실측에서 전부 격식체 글은 좋아요 1.04배로 평균 이상이고, #137 에서 이
# 규칙이 정규식 리젝의 63% 를 차지했다. 이제 계약은 반대다 — 격식체 3문장은
# 통과해야 하고, 같은 어미 4회 반복(어미반복)은 계속 리젝돼야 한다.
def check_formal_only() -> None:
    sec("격식체 허용 vs 어미반복 필터")
    ok_body = ("로보티즈가 어제 20.32% 올랐습니다. 종가는 299,000원입니다. "
               "거래대금은 2,990억원이었습니다.")
    errs = filters.check(ok_body, "등락률: 20.32%", "fact_note", "reaction", "fact_note")
    if any(e.startswith(("어미단조", "어미반복")) for e in errs):
        fail(f"격식체 3문장이 어미 규칙에 걸린다 — 반응 좋은 글을 버린다: {errs}")
    else:
        ok("격식체 3문장 통과")
    rep = ("로보티즈가 20.32% 올랐습니다. 종가는 299,000원까지 갔습니다. "
           "거래대금은 2,990억원을 기록했습니다. 5거래일로는 31% 올랐습니다.")
    errs = filters.check(rep, "등락률: 20.32%", "fact_note", "reaction", "fact_note")
    if not any(e.startswith("어미반복") for e in errs):
        fail("같은 어미 4회 반복이 필터에 안 걸린다 — 획일성 방어가 죽어 있다")
    else:
        ok("같은 어미 4회 반복을 필터가 잡는다")
