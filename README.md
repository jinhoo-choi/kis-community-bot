...
## 흐름 (멀티 LLM)
```
수집 → 캐시 재사용 → 하드 게이트 → 종목·게시판 매핑 → 중복제거
  → [0] flow 확정 사실로 검증 문장틀 50+15건 준비 (API 없음)
  → 후보 부족 시에만 5건씩 검색 보강
  → [1] 단계별 작성 : LLM 승인 35건까지 최초 60건, 이후 실수율 기반 10~60건씩 호출
  → [2] 결정형 검수 : 근거 수치·derived fact·문체 계약 확인
  → [3] 교차 심사   : 작성자와 다른 모델이 6축 채점
  → [4] 배포 판정   : LLM 우선 + 평시 문장틀 최대 15건 / 장애 시 부족분 보충
...
| **API-last** | 확정 사실 reserve를 먼저 만들고, 새 원본 후보를 우선하며 재작성·검색 보강은 부족할 때만 실행 |
| **50건 준비** | 날짜가 유효한 flow 원본 65건이면 22종 결정형 문장틀로 API 장애 중에도 50개 본문 준비 |
...
# 2. 단위 회귀 + 문장틀 50건 보장 + 설정·프롬프트·계약 전수 감사
...
| API 비의존 보충 | flow `template_v1` 22종 (평시 최대 15건, 보장 모드 부족분) |
...
문장틀은 `data_focus`·`fact_note`·`brief_report`만 사용하며, 임의 승인 플래그가 아니라
원본 facts와 본문을 매번 재구성해 검증합니다. 텔레그램 카드의 `생성 경로` 줄에서 LLM과
검증 문장틀을 구분하며 이 표시는 복사 본문에는 포함되지 않습니다.
...
**원문에 없는 숫자(환각)** → 리젝 큐에 보관. 아직 쓰지 않은 원본 후보를 먼저 처리하고,
목표 미달일 때만 다른 provider로 1회 재생성
