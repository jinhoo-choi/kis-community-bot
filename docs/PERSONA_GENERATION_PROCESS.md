...
- `term_guide`는 `facts.GLOSSARY`의 검증 문장 17종이 입력에 붙을 때만 활성화됩니다.
- LLM 페르소나와 별도로 flow 한정 결정형 문장틀 22종을 먼저 준비합니다. 평시에는 최대
  15건만 쓰고, LLM 공급이 부족할 때만 50건 보장 모드로 부족분을 채웁니다.

## 2. 전체 흐름

```mermaid
flowchart TD
    A["게이트 통과 후보"] --> R["flow 문장틀 reserve 65"]
    A --> B["Fact Slot·Angle 추출"]
...
| 단계 | 실제 코드 | 동작 |
|---:|---|---|
| 0 | `template_reserve.py` | flow 원본으로 API 호출 전 50+15건을 준비하고 원문·필터·grounding을 재검증 |
...
### 결정형 flow 문장틀의 페르소나 계약

문장틀은 위 10개 LLM 페르소나 중 `data_focus`, `fact_note`, `brief_report` 세 ID만 빌려
배포 다양성 통계와 길이 경계를 공유합니다. LLM 프롬프트를 실행하지 않고 다음 계약으로
본문을 다시 만들 수 있을 때만 유효합니다.

| 항목 | 계약 |
|---|---|
| 적용 | `flow` 한정 |
| 종류·반복 | 22종, 동일 `template_id` 3건 이하 |
| 분량 | 3문장, 70~200자 |
| 주장 | 등락률·종가·관계값, 최대 3개 |
| 관계값 | 20일 평균 대비 거래량·5거래일 누적·고가 대비 마감 위치 중 1개 이상 |
| 질문 | 금지 |
| 평시 | LLM 승인본 35건 우선, 문장틀 최대 15건 |
| 보장 모드 | LLM 승인본 부족분만 최대 50건, 세 페르소나 각각 17건 이하 |
| 검증 | facts+본문+template_id 재구성 → `filters` → `grounding_errors` → `uses_derived` |
...
| 문장틀 | LLM 점수를 만들지 않고 매 판정 때 renderer 원문 일치와 결정형 계약을 다시 검사 |
...
비용 계측 후 실발송 [34693897268](https://github.com/jinhoo-choi/kis-community-bot/actions/runs/34693897268)은
후보 43건을 `[20,14,9]`로 전부 생성해 필터 통과 17건, 최종 발송 5건이었습니다. 이 실행에서
`data_focus` 2건, `fact_note` 2건, `check_list` 1건이 선택됐고 5건 모두 Claude 작성이었습니다.
고저 차이를 상승폭처럼 바꾼 두 표현과 마감·거래량 비문은 `3a64a99`로 회귀 고정했습니다.
...
| 13 | 낮은 LLM 수율과 API 전면 장애가 50건 조건과 충돌 | flow 문장틀 22종, 65건 reserve, 정상/보장 모드, 생성 경로 표기 | `4c5da44` |
...
| E2E | 20/20 |
| 실제 시세 캐시 dry-run | reserve 65/65, 최종 50/50 |
...
3. `term_guide`가 계속 0이면 `facts.GLOSSARY`가 매칭되는 공시가 실제 후보에 있었는지 먼저
   확인합니다. 매칭 입력이 없는데 가중치만 올리지 않습니다.
