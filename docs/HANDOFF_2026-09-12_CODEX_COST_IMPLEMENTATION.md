...
이 문서 작성 당시 남아 있던 flow template 22종, 50+15 reserve, 보장 모드 검증기는
`4c5da44`에서 구현됐습니다. 실제 시세 캐시 234건으로 reserve 65/65, API 장애 dry-run
50/50을 확인했습니다. 상세 계약·실발송 비용·검증 결과는
...
2. 새 코드로 테스트 채널 5건, 그다음 50건을 전송합니다.
3. 새 계측으로 호출·token·비용·수정률을 기준선과 비교합니다.
